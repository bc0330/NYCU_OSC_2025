...
void fdt_traverse(void (*callback)(char *)) {
    ...
    __dtb_end = (void *) (unsigned long) fdt + be2le(fdt->totalsize);
    ...
    while (struct_off < struct_size) {
        ...
        if (token == FDT_BEGIN_NODE) {
            ...
            if (len > 0) {
                ...
                } else if (strcmp(prop_name, "linux,initrd-end") == 1) {
                    rootfs_end = (char *) be2le(*(uint64_t *) (struct_addr + struct_off));
                    uart_send_string("Found initramfs end at ");
                    uart_send_num((uint64_t) rootfs_end, "hex");
                    uart_send('\n');
                }

...
void receive_handler(void) {
    uart_send_string("receive handler\r\n");
    char c = get32(AUX_MU_IO_REG) & 0xFF;
    unsigned int int_enable = get32(AUX_MU_IER_REG);
    put32(AUX_MU_IER_REG, int_enable & ~0b01);                               // disable receive interrupt temporarily
    if ((read_rear + 1) % MAX_BUFFER_SIZE == read_front) {
        uart_send_string("Buffer is full\r\n");
    } else {
        read_buffer[read_rear] = c;
        read_rear = (read_rear + 1) % MAX_BUFFER_SIZE;
    }
    put32(AUX_MU_IER_REG, int_enable);                                       // re-enable receive interrupt
}

void transmit_handler(void) {
    uart_send_string("transmit handler\r\n");
    unsigned int int_enable = get32(AUX_MU_IER_REG);
    put32(AUX_MU_IER_REG, int_enable & ~0b10);                               // disable transmit interrupt temporarily
    if (write_front != write_rear) {                                         // buffer is not empty
        put32(AUX_MU_IO_REG, write_buffer[write_front]);
        write_front = (write_front + 1) % MAX_BUFFER_SIZE;
        put32(AUX_MU_IER_REG, int_enable);                                   // re-enable transmit interrupt
    } else {
        // If the buffer is empty, disable the transmit interrupt
        // uart_send_string("Buffer is empty\r\n");
        put32(AUX_MU_IO_REG, 0); 
    }
}

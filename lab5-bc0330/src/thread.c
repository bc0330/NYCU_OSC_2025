...
thread_t *thread_create(void (*function)(void), int priority, void (*user_prog)(void), size_t prog_size) {
    thread_t *thread = (thread_t *)allocate(sizeof(thread_t));
    if (!thread) {
    ...
    // unsigned long *sp = (unsigned long *)thread->usr_stack;
    ...
    thread->context[11] = (unsigned long)thread_start; // Set the function to execute in the context
...
void schedule(void) {
    ...
    if (next_thread != NULL) {
        ...
        // uart_send_num(next_thread->id, "dec");
    ...
    switch_to(prev_thread->context, next_thread->context); // Switch to the next thread
}

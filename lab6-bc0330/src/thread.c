...
thread_t *thread_create(void (*function)(void), int priority, void (*user_prog)(void), size_t prog_size) {
    ...
    // thread->usr_stack_base = allocate(thread_stack_size); // Allocate stack for the thread
    thread->usr_stack_base = 0;
    ...
    // memset((char *)thread->usr_stack_base, 0, thread_stack_size); // Initialize the stack to zero
    memset((char *)thread->kernel_stack_base, 0, thread_stack_size); // Initialize the kernel stack to zero

    // for (int i = 0; i < 4; ++i) {
    // mappages(thread->pgd, 0xFFFFFFFFE000, vtop((unsigned long)thread->usr_stack_base), PD_USR_ACCESS); // Map the user stack pages
    // }
    thread->usr_stack = thread->usr_stack_base + thread_stack_size; // Set the stack pointer to the top of the stack
    ...
    mem_region_t prog_region = {
        .start = 0,
        .end = prog_size + (PAGE_SIZE - prog_size % PAGE_SIZE), // Align the end address to the next page boundary
        .prot = PROT_READ | PROT_WRITE | PROT_EXEC,
        .next = &stack_region
    };
    thread->regions = &prog_region; // Set the memory regions for the thread

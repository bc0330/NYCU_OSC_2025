...
static int mkdir(struct vnode* dir_node, struct vnode** target,
                  const char* component_name);
struct vnode_operations devfs_vops = {
    .lookup = lookup,
    .create = create,
    .mkdir = mkdir,
};
...
static int setup_mount(struct filesystem* fs, struct mount* mount) {
    ...
    devfs_internal_t *inter = allocate(sizeof(devfs_internal_t));
    inter->child_count = 0;
    inter->size = 0;
...
static int write(struct file* file, const void* buf, size_t len) {
    ...
    devfs_internal_t* inter = (devfs_internal_t*)node->internal;
...
static int lookup(struct vnode* dir_node, struct vnode** target,
                  const char* component_name) {
    devfs_internal_t* inter = (devfs_internal_t*)dir_node->internal;
    for (int i = 0; i < DIR_ENTRIES; ++i) {
...
static int mkdir(struct vnode* dir_node, struct vnode** target,
    ...
    new_node->mount = NULL;
    new_node->v_ops = &devfs_vops;
    new_node->f_ops = &devfs_fops;      
    new_node->parent = dir_node;
    new_node->internal = allocate(sizeof(devfs_internal_t));

    devfs_internal_t* inter = (devfs_internal_t*)new_node->internal;
    ...
    devfs_internal_t* dir_inter = (devfs_internal_t*)dir_node->internal;
    int i;
